...
ui <- shiny::navbarPage(
  ...
  shiny::tabPanel(
    ...
    shiny::fluidRow(
      shiny::h4(
        class = "text-center", 
        "The goal is for the blue bar to meet the green line."
      )
    )
  ...
  shiny::tabPanel(
    ...
    shiny::p("If you are havng a problem you can write to us about your issue https://github.com/CWWhitney/calibration_app/issues"),
...
server <- function(input, output, session) {
  ...
  shiny::modalDialog(
    ...
    "Please type just one name per field and avoid special letters e.g. ä ü é or symbols like -.",
    shiny::tagList(
      shiny::div(
        shiny::textInput(
  ...
  shiny::observeEvent(input$next_btn, {
    ...
    if ((rctv$current_question_type == "range") & (rctv$current_response_1 >= rctv$current_response_2)) {
      ...
      modal_text_1 <-  "You answered (Lower 90%):"
      
      
      # Create the second modal text segment
      modal_text_2 <-"You answered (Upper 90%):"
